		...
		public void execute() {
			...
			while (!isShutdown && (!isStop || taskQueue.size() > 0)) {
				try {
					...
					// System.out.println("Tak a task to execute , now the
					// length of task queue is " + taskQueue.size());
					task.execute();
				} catch (InterruptedException e) {
					e.printStackTrace(); // To change body of catch statement
											// use File | Settings | File
	...
	public void start() {
		...
		if (isStop && !isRunning) {
			...
			throw new IllegalStateException(
					"This executor is still alive or running, please stop or shutdown immediately and try it again later");
		}
	}
