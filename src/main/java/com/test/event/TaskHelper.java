...
public class TaskHelper {
	public static TaskEventEmitter createIOTask(TaskExecutor executor, String fileName) {
		...
		task.on("open",event -> {
			String fileName2 = (String) event.getArgs()[0];
			System.out.println(Thread.currentThread() + " - " + fileName2 + " has been opened.");
		});
		...
		task.on(IOException.class.getName(), event ->{
			...
			String fileName2 = (String) args[1];
			System.out.println(Thread.currentThread() + " - An IOException occurred while reading " + fileName2
					+ ", error: " + e.getMessage());
		});
		task.on("close", event ->{
			String fileName2 = (String) event.getArgs()[0];
			System.out.println(Thread.currentThread() + " - " + fileName2 + " has been closed.");
		});
